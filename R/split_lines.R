...
split_lines <- function(lines, points, snap_tol = 10, tmp_crs = NULL) {
	...
	lines <- lines |> sf::st_collection_extract("LINESTRING") |> sf::st_cast("LINESTRING")
...
segment_lines <- function(lines, points, snap_tol) {
	if (is.character(lines)) {
		lines <- sf_from_parquet(lines)
	}
	if (is.character(points)) {
		points <- sf_from_parquet(points)
	}
	...
	lines_segmented <- lines_segmented |>
		...
		dplyr::select(
			-c(from, to, from_yr_open, to_yr_open, from_infill, to_infill, from_running, to_running)
		) |>
