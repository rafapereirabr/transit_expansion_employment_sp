...
#' factor data into quantiles
#' @param x a `numeric` vector
#' @param weights a `numeric` vector containing weights
#' @param \dots other arguments passed on to `Hmisc::wtd.quantile`
#' @importFrom Hmisc wtd.quantile

quantilize <- function(x, weights = NULL, n_qt, relative_labs = FALSE, ...) {
	if (is.null(weights)) {
		quantiles <- quantile(x, seq(0, 1, 1 / n_qt), ...)
	} else {
		quantiles <- Hmisc::wtd.quantile(x, weights, seq(0, 1, 1 / n_qt), ...)
	}

	## need that because of units with 0 weight
	quantiles[1] <- min(x)
	quantiles[length(quantiles)] <- max(x)

	labs <- if (relative_labs) {
		paste("Up to", names(quantiles[-1]))
	} else {
		ceiling(quantiles[-1])
	}

	cut(x, quantiles, labs, TRUE) |>
		factor()
}
