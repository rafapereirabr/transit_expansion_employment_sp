...
write_naive_table <- function(panel, save_dir = NULL, balanced = F, format = "docx") {
	...
	tbl <- panel |>
		...
		summarise(
			formal = if_else(sum(emp_31dez, na.rm = T) > 0, 1, 0),
			travel_time = mean(travel_time_p50, na.rm = T)
		) |>
		...
		summarise(
			...
			mean_travel_time = mean(travel_time * formal, na.rm = T)
		) |>
		collect()
	...
	tinytable::save_tt(tiny_tbl, file_path, overwrite = T)
	return(file_path)
}
