...
discover_feeds <- function(dir) {
	candidates <- list.files(dir, pattern = "\\.zip$", recursive = TRUE, full.names = TRUE) |>
		sort()
	candidates[
		str_detect(basename(candidates), regex("gtfs|sptrans", ignore_case = TRUE)) &
...
write.csv(
	bus_route_history,
	file.path(output_dir, "sapopemba_bus_route_history.csv"),
	row.names = FALSE
)
...
timeline <- bind_rows(
	...
	bus_hive_history |>
		transmute(
			feed_id,
			reference_date,
			value = median_headway_minutes,
			metric = "Sapopemba bus median headway (minutes)"
		),
	city_bus_history |>
		transmute(feed_id, reference_date, value = weighted_speed_kmh, metric = "City bus speed (km/h)"),
	city_bus_history |>
		transmute(
			feed_id,
			reference_date,
