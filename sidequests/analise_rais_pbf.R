...
cad <- purrr::map(
	...
	function(x) {
		...
		df <- df |>
			mutate(
				code_state = as.double(co_uf),
				code_muni = cd_ibge_cadastro,
				cpf = nu_cpf_pessoa,
				femin_cad = as.double(co_sexo_pessoa - 1),
				ano_nasc = lubridate::year(dt_nasc_pessoa),
				.keep = "unused"
			) |>
...
summary_cad |>
	...
	facet_wrap(vars(ano)) +
	labs(
		title = "Bolsa Família x emprego formal",
